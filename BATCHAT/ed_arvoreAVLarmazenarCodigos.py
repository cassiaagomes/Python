...
class AVLTree(object): 
    ...
    def __leftRotate(self, p:Node)->Node: 
        """
        Realiza a rotação 'à leftuerda' tomando o no 'p' como base
    ...
    def __busca(self, valorProcurado, nodeAtual:Node)->bool:
        
        if nodeAtual is None: 
            return False

        elif nodeAtual.value==valorProcurado:
            return True
        
        else:
            if valorProcurado < nodeAtual.value: 
                return self.__busca(valorProcurado, nodeAtual.left)
            
            elif valorProcurado > nodeAtual.value:
                return self.__busca(valorProcurado, nodeAtual.right)
