...
class ChainHashTable1:
    ...
    def get(self, key:int):
        ...
        slot = self.__hash(key) #gqwgtdqug     
        for entry in self.table[slot]:
            if entry.key == key:
                return entry.value
        return -1
    ...
    def remove(self, key: int):
        """Remove a entrada correspondente à chave da tabela hash."""
        slot = self.__hash(key)
        for entry in self.table[slot]:
            if entry.key == key:
                self.table[slot].remove(entry)
                
                return
        raise ValueError("Chave especificada não encontrada na tabela hash")
